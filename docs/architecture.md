...
| --- | --- | --- |
| `bearer()` (`@elysiajs/bearer`) | plugin, used per router | Exposes the raw `Authorization: Bearer <token>` value as `bearer` in the handler context. Used alone where a route only needs to decode the token (e.g. cosmetics, loadout, admin routers). |
...
- Ranked tiers, points, ratings, annulment, bans, cosmetics and tournaments as domain concepts: `docs/domain/` (planned).
