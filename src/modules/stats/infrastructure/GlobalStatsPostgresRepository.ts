...
export class GlobalStatsPostgresRepository implements GlobalStatsRepository {
	...
	async getDuelsPerBanList(season: number): Promise<BanListBreakdown[]> {
		...
		const totalSeasonDuels = result.reduce(
			(sum: number, row: StatsRow) => sum + row.total_duels,
			0,
		);
		...
		return result.map((row: StatsRow) => ({
			...
			percentage:
				totalSeasonDuels > 0
					? parseFloat(((row.total_duels / totalSeasonDuels) * 100).toFixed(1))
					: 0,
			popularity:
				totalSeasonDuels > 0
					? Math.min(100, Math.round((row.total_duels / totalSeasonDuels) * 100 * 3))
					: 0, // Scale for UI bar
		}));
	}
