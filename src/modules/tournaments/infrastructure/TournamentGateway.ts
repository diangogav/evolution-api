...
export class TournamentGateway implements TournamentRepository {
	...
	async enrollTournament({
		...
		const response = await fetch(
			`${config.tournaments.apiUrl}/tournaments/${tournamentId}/entries`,
			{
				method: "POST",
				headers: { "Content-Type": "application/json" },
	...
	async withdrawTournament(tournamentId: string, participantId: string): Promise<void> {
		const response = await fetch(
			`${config.tournaments.apiUrl}/tournaments/${tournamentId}/entries/${participantId}`,
			{
				method: "DELETE",
			},
		);
	...
	async editMatchResult(
		...
		const response = await fetch(
			`${config.tournaments.apiUrl}/tournaments/${tournamentId}/matches/${matchId}/result`,
			{
				method: "PUT",
				headers: { "Content-Type": "application/json" },
	...
	async publishTournament(tournamentId: string): Promise<void> {
		const response = await fetch(
			`${config.tournaments.apiUrl}/tournaments/${tournamentId}/publish`,
			{
				method: "PUT",
			},
		);
	...
	async completeTournament(tournamentId: string): Promise<void> {
		const response = await fetch(
			`${config.tournaments.apiUrl}/tournaments/${tournamentId}/complete`,
			{
				method: "PUT",
			},
		);
	...
	async cancelTournament(tournamentId: string): Promise<void> {
		const response = await fetch(
			`${config.tournaments.apiUrl}/tournaments/${tournamentId}/cancel`,
			{
				method: "PUT",
			},
		);
	...
	async confirmTournamentEntry(tournamentId: string, participantId: string): Promise<void> {
		const response = await fetch(
			`${config.tournaments.apiUrl}/tournaments/${tournamentId}/entries/${participantId}/confirm`,
			{
				method: "PUT",
			},
		);
