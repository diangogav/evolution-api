...
export const rankedTiersRouter = new Elysia({ prefix: "/ranked-tiers" }).get(
	...
	({ query }) => getTierCatalog.run(query.banListName),
	{
		detail: {
			...
			responses: {
				200: {
					description: "Tier catalog retrieved successfully",
					content: {
						"application/json": {
							// The catalog is a compile-time constant, so the live default
							// ladder is the example.
							example: getTierCatalog.run("TCG"),
						},
					},
				},
			},
