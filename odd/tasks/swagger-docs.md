...
- [x] 3.3 Operations (`docs/operations.md`; branch `docs/knowledge-base-03-operations`, delegated): environment variables (names and purpose only), running locally, migrations per DataSource, seeds and scripts, deployment notes. Done: 6c7c610 (page, index and forward references updated), 3d345eb (rating script guard corrected), e98f535 (outdated `database-scripts/` removed at the user's request; the page explains how `player_stats` is maintained and that no rank-keyed full rebuild exists), plus the local shared-schema setup step; native review review-228191e398497d10 (medium because of the SQL deletion, consent granted) approved and acknowledged, its four notes fixed; risky claims checked against config, index.ts and the Dockerfile.
