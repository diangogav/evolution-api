...
- [x] 3.2 Domain concepts (`docs/domain/`; branch `docs/knowledge-base-02-domain`, delegated): seasons and ranks, points ledger, Elo ratings, match annulment and reversal, ranked tiers (link the existing guide), bans, cosmetics and entitlements, tournaments and the upstream service. Done: a25c99c, 699 lines in 7 pages (index and glossary, seasons/ranks/points, points ledger and ratings, match annulment, moderation, cosmetics, tournaments) plus index and architecture link updates; game-server ownership confirmed read-only in EDOpro-server-ts (matches, applied ledger and rating rows); constants and cited files spot-checked; assessment passive, no review due. New behavior facts: equipping does not check `cosmetic.active`; `RankEntity.onlyCurrent` is unused by this API.
- [x] 3.3 Operations (`docs/operations.md`; branch `docs/knowledge-base-03-operations`, delegated): environment variables (names and purpose only), running locally, migrations per DataSource, seeds and scripts, deployment notes. Done: 6c7c610 (page, 224 lines, index and forward references updated), 3d345eb (parent fix: stats SQL scripts marked outdated, rating script guard corrected); risky claims checked against config, index.ts, Dockerfile and database-scripts; assessment passive, no review due.
  - [ ] 3.3-F1 (ops) the four `database-scripts/*.sql` still target `player_stats.ban_list_name`, which the rank cutover replaced with `rank_id`; two of them delete a season's rows first. Retire or rewrite them.
